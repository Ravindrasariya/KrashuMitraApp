#!/usr/bin/env node
// Task #126 — Historical weather export for 5 additional Rajasthan/Gujarat
// cities. Mirrors script/historical-weather-export.mjs (same column schema,
// same 06:00 IST morning-snapshot contract for hourly fields, same bounded
// retry, same per-chunk JSON checkpointing) but writes a SEPARATE workbook
// at exports/historical_weather_rajasthan_gujarat_2010_to_today.xlsx so the
// existing 16-city file is left untouched (user requirement).
...
// Pinned to match the existing 16-city file's exact range (no stray
// 2026-05-17 rows from cities whose fetch crosses the IST midnight boundary).
...
const OUT_PATH  = resolve(process.cwd(), "exports/historical_weather_rajasthan_gujarat_2010_to_today.xlsx");
...
async function main() {
  ...
  // Validate uniform grid before writing.
