...
import { pgTable, pgSequence, serial, integer, text, varchar, timestamp, boolean, date, uniqueIndex, numeric, doublePrecision, jsonb } from "drizzle-orm/pg-core";
...
// Task #108: Bills generated by sellers from the marketplace listing detail
// dialog. `sequenceNo` is allocated from a dedicated `bill_seq` so the
// number is globally unique across all sellers (per user requirement —
// "unique for all the sellers together"). The same sequence value is used
// to format both the order number (ON{YYYYMMDD}{N}) and the invoice number
// (IN{YYYYMMDD}{N}) on a single bill, so the seller sees a consistent
// pair. `payload` stores the full draft snapshot so a future reprint task
