...
- For marketplace listings, the OG image is a per-listing **composed share card** served at `GET /api/marketplace/:id/share-image` (1200×630 JPEG). Layout (Task #77): listing's first photo cover-fit on the top 490 px (~78%), white card band on the bottom 140 px (~22%) with the in-app category badge in the top-left, location pin + village/district in the bottom-left, and `km.krashuved.com` in the bottom-right. The price/variety/quantity summary line that used to sit inside the band was removed because WhatsApp / Facebook already render the same content from `og:description` directly below the picture. When the listing has no photo, the brand cover is used as the top background. Composer lives in `server/share-image.ts` (sharp + SVG overlay); response is cached 1h via `Cache-Control` and a strong `ETag` derived from the listing's content fields + photo ids + a layout-version constant (`lay`), so any of (a) listing edits, (b) photo changes, or (c) future SVG layout tweaks invalidate the cache automatically.
