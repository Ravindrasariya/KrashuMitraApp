...
- For listings without a photo (or for the homepage), the cover is used as the OG image; OG `image:width`, `image:height`, `image:type` and `image:secure_url` tags are emitted alongside it. When a listing has a photo, the listing's first photo URL is used instead.
- **Production env vars (must be set on the VPS / pm2 ecosystem so shared URLs are short and OG images use the canonical domain):**
  - `PUBLIC_BASE_URL=https://km.krashuved.com` — used by the server when building absolute OG `og:url` and `og:image` URLs.
  - `VITE_PUBLIC_BASE_URL=https://km.krashuved.com` — read at build time by the client's `composeShareInfo` so the URL appended to the WhatsApp/email body is always `https://km.krashuved.com/marketplace?listing=<id>` even when sharing from a tester's dev environment.
  - Optional: `SHARE_ALLOWED_HOSTS=km.krashuved.com` — restricts host-derived origins (when `PUBLIC_BASE_URL` is unset) to known canonical hosts.
