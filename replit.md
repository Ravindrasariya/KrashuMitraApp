...
- For marketplace listings, the OG image is a per-listing **composed share card** served at `GET /api/marketplace/:id/share-image` (1200×630 JPEG). Layout: listing's first photo cover-fit on top 70%, white card band on bottom 30% with the in-app category badge, summary line (price · variety/duration · quantity), location pin + village/district, and `km.krashuved.com` footer. When the listing has no photo, the brand cover is used as the top background. Composer lives in `server/share-image.ts` (sharp + SVG overlay); response is cached 1h via `Cache-Control` and an `ETag` derived from the listing's `updatedAt` and photo count, so cache invalidates whenever the listing or its photos change.
- Hindi (Devanagari) text in the composed share card is rendered via the bundled `server/assets/fonts/NotoSansDevanagari-Regular.ttf`. On first request, the composer copies that TTF into `~/.fonts/` and runs `fc-cache -f` once so fontconfig (used by sharp/librsvg) can resolve the `Noto Sans Devanagari` family. The font ships in the repo so prod (Hostinger VPS) doesn't depend on system Noto fonts. The production build script (`script/build.ts`) copies `server/assets/` → `dist/assets/` after esbuild so the TTF sits next to `dist/index.cjs` at runtime; the composer resolves it via `path.resolve(__dirname, "assets/fonts/...")` (CJS bundle) or `import.meta.dirname` (tsx dev).
- **Prod VPS dependency**: the deploy host needs `fontconfig` installed (`apt install fontconfig` on Debian/Ubuntu) so `fc-cache` and font lookups work. Without it the TTF will be present but Devanagari glyphs render as tofu boxes. No restart needed after install — the composer re-runs `fc-cache -f` on its next cold start.
...
- **Versioned canonical og:url** (Task #73): when a crawler hits `/marketplace?listing=<id>&v=<shareVersion>`, the server-injected `og:url` echoes the same `&v=` token (sanitized to `[A-Za-z0-9_-]{1,64}`). Without this, WhatsApp's scraper would normalize its preview cache to the un-versioned canonical and collapse every versioned share into one (broken) cache entry. Same content → same token → same canonical → reuses the cached rich card; edited content → new token → new canonical → fresh scrape.
...
- **Persistent on-disk share-image cache** (Task #73): composed share-image JPEGs are written to `<install-root>/.share-image-cache/l<id>-<createdAtMs>-<sigHash>.jpg` (atomic write via tmp + rename, content-addressed by the strong ETag key, with stale per-listing files cleaned up on each new write). Survives restarts, so WhatsApp's first scrape of a freshly-versioned URL is a static-disk read instead of cold sharp work (~250 ms saved). The directory is created lazily on first compose and gitignored. Override the location via `SHARE_IMAGE_CACHE_DIR=/path/to/dir` if the install root isn't writable. Skipped when `NODE_ENV=test`.
