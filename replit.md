...
## Plot Health Check (Digital Clinic)
- A Digital Clinic service that shows Sentinel-2 satellite vegetation indices for a chosen plot over an interactive Leaflet map plus a live weather panel. Bilingual Hi/En. Default map center: Ujjain (23.1765, 75.7885).
- **Data source**: Copernicus Data Space Ecosystem (CDSE). Server module `server/sentinel.ts` does OAuth2 client-credentials token caching, evalscripts (true-color, NDVI/NDRE/NDMI viz with SCL cloud masking, 3-band FLOAT32 stats), `renderTile` (Processing API, EPSG:3857 XYZ), `findAcquisition` (Catalog STAC, cloud-filtered), and `getStats` (Statistical API over the lot box). Requires secrets `CDSE_CLIENT_ID` / `CDSE_CLIENT_SECRET` (free Copernicus account → Sentinel Hub OAuth client). When absent, `hasCredentials()` is false and the UI shows a "setup needed" card; endpoints return 503.
- **Endpoints** (all `isAuthenticated`): `GET /api/plot-health/config` (creds flag + Ujjain center + zoom caps + box sizes), `POST /api/plot-health/analyze {lat,lng,boxSizeM,date|"latest"}` → `{resolvedDate, acquisitionDate, cloudCover, stats, noClearImage}` and saves a `service_requests` row (`serviceType="plot_health"`, `status="closed"`, payload JSON in `aiDiagnosis`), `GET /api/plot-health/tiles/:index/:z/:x/:y.png?date=` → PNG. Both stats and tiles use bounded in-memory LRU caches. Tile params are strictly validated: integer-only z/x/y, `z` within zoom caps, `0 ≤ x,y < 2^z`, and date bounded to the Sentinel-2 era (≥ 2015-06-01, ≤ today) to prevent authenticated cache-miss amplification against CDSE.
- **Frontend**: `client/src/components/plot-health.tsx` — location picker (GPS / manual lat,lng / tap-on-map), box-size select (default 50 m), date picker + "latest clear image" toggle, layer toggle (True color / NDVI / NDRE / NDMI), Esri World Imagery base + index overlay tile layer with opacity slider + color-ramp legend + ~10 m/pixel resolution note, per-lot stats panel (mean NDVI/NDRE/NDMI, acquisition date, cloud %), and a client-side Open-Meteo weather panel (temp, humidity, rain, wind speed + direction compass arrow + gusts, 3-day forecast). All strings via `ph*` i18n keys.
- **History / reopen**: plot_health rows in the Digital Clinic request history are expandable (coords, box, image date, cloud %, mean indices) with a button that rehydrates the saved result back into the interactive map (`safeParsePlot` + remount key in `client/src/pages/digital-clinic.tsx`).
- **Crop + stage health verdict**: two dropdowns above the lat/long row let the farmer pick "Crop in the field" (10 crops + Others (Generic) + Barren (No Crop)) and a crop-dependent "Crop Stage" (hidden for Barren). On analyze, the measured NDVI/NDRE/NDMI means are compared against seeded healthy ranges per crop+stage in the `crop_stage_references` table (auto-seeded at startup; `PLOT_CROP_STAGES`/`PLOT_CROP_KEYS` in `shared/schema.ts` are the single source of truth shared by seed, validation, and dropdowns). The result is a bilingual Hi/En verdict card (healthy → generic guidance; below → warning naming the weak index: canopy/nitrogen/moisture). Logic in `computeCropAssessment`/`assessIndex` (`server/routes.ts`); a non-barren crop requires a stage before analyze. DDL: `migrations/0011_crop_stage_references.sql`.
- **Search audit log**: every analyze call writes one row to the dedicated `plot_health_searches` table — requested/resolved date, lat/long, box size, crop + stage, NDVI/NDRE/NDMI mean/min/max, cloud cover, valid fraction, verdict, and a `no_clear_image` flag (no-clear-image misses are logged with null index values). Backend-only audit trail (`storage.createPlotHealthSearch`); logging errors are swallowed so they never break the analyze response. DDL: `migrations/0012_plot_health_searches.sql`.
- Bhulekh / plot-boundary overlays are intentionally deferred (not part of this feature).
