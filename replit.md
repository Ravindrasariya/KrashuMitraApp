...
- **Marketing Banners**: Admin-managed banner carousel on home page. Supports two types: text-based (heading/subheading/description in Hi+En with light green gradient) and image-based (uploaded image with caption overlay). Auto-rotating carousel with dot indicators, prev/next arrows. Admin can create, edit, delete, reorder, and toggle active/inactive. Desktop banners are padded with `md:mx-8` and 10% taller (`md:min-h-[220px]`); mobile unchanged. Banners stored in `banners` table; images stored as base64 with 5MB limit. Admin management via "Manage Banners" tab on admin page.
